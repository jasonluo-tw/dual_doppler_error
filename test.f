...
c      close(10)
     ...
     &   domain)
       ...
       real*8 w_vari(nxmax,nymax,21)
       real*8 u_vari(nxmax,nymax,20)
       real*8 v_vari(nxmax,nymax,20)
       real plot_w(nxmax,nymax)
       real   cont_index(16)
...
cc     the point where the radial wind is observed
      ptx=float(nxx)
      pty=float(nyy)
      do 304 k=20,1,-1  ! nlevel

      pheight=float(k*500) !the point height
      kapa=-1./(exp(-pheight/Hs))*
     &    ((exp(-(pheight+500)/Hs)-exp(-pheight/Hs))/delta_z)
...
c       read(*,*)nlevel
      ...
      print *,'~~~~~'
       do i=1,nxmax
         write(15,*)(plot_w(i,j),j=1,nymax)
       enddo

       do i=1,16
        cont_index(i)=5.0+float(i-1)*0.3
       enddo

      call contour(plot_w,nxmax,nymax,dummy,cont_index,16,1.5,ca7,16,4)
c       do i=1,16
c        cont_index(i)=5.0+float(i-1)*0.1
c       enddo
c      call contour(plot_w,nxmax,nymax,dummy,cont_index,32,1.5,ca7,16,4)
      ...
      call plchhq(0.5,ft+0.04,'Domain '//trim(dm_ti),0.03,0.,0.)
      write(lv_ti,'(f4.1)')float(nlevel)*0.5
c      print *,lv_ti
